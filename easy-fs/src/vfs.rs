...
impl Inode {
    ...
    /// Create a hard link: add `new_name` as another directory entry pointing to inode of `old_name`.
    /// Returns 0 on success, -1 on error (same name, or old_name not found).
    pub fn link(&self, old_name: &str, new_name: &str) -> isize {
        if old_name == new_name {
            return -1;
        }
        let mut fs = self.fs.lock();
        // Find the inode_id of old_name
        ...
        // Add a new directory entry pointing to the same inode
        self.modify_disk_inode(|root_inode| {
            let file_count = (root_inode.size as usize) / DIRENT_SZ;
            let new_size = (file_count + 1) * DIRENT_SZ;
            self.increase_size(new_size as u32, root_inode, &mut fs);
        ...
        let (target_block_id, target_block_offset) = fs.get_disk_inode_pos(old_inode_id);
    ...
    /// Remove the directory entry for `name`. If nlink drops to 0, also free the inode and data blocks.
    /// Returns 0 on success, -1 if the file does not exist.
    pub fn unlink(&self, name: &str) -> isize {
        let mut fs = self.fs.lock();
        // Find entry index and inode_id for `name`
        let result = self.read_disk_inode(|root_inode| {
            assert!(root_inode.is_dir());
            let file_count = (root_inode.size as usize) / DIRENT_SZ;
            let mut dirent = DirEntry::empty();
            for i in 0..file_count {
        ...
        let (entry_idx, target_inode_id) = match result {
            Some(v) => v,
            None => return -1,
        };
        // Decrement nlink on the target inode; check if we need to delete it
        let (target_block_id, target_block_offset) = fs.get_disk_inode_pos(target_inode_id);
        let remaining_nlink = get_block_cache(
        ...
        // Remove the directory entry by overwriting it with the last entry,
        // then shrinking the directory size by DIRENT_SZ.
        self.modify_disk_inode(|root_inode| {
            let file_count = (root_inode.size as usize) / DIRENT_SZ;
        ...
        // If nlink reached 0, free data blocks and the inode
        if remaining_nlink == 0 {
            // Clear data blocks of the target inode
            ...
            for block_id in blocks_to_dealloc {
    ...
    /// Get stat info: returns (inode_id, is_dir, nlink)
    pub fn stat(&self) -> (u64, bool, u32) {
        self.read_disk_inode(|disk_inode| {
            (self.inode_id as u64, disk_inode.is_dir(), disk_inode.nlink())
        })
    }
